...
test("does not treat an ordinary left-voicemail note as a departed contact", () => {
  const report = reconcilePeople(
    [],
    [
      {
        ...
        organizationName: "Example Build Ltd",
        labels: [],
        notes: ["Left voicemail with reception on 2026-07-28"],
      },
    ],
  );

  assert.equal(report.counts.departed_or_stale, 0);
  assert.equal(report.counts.pipedrive_only, 1);
});
