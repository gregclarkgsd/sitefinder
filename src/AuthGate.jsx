...
export default function AuthGate({children}) {
  ...
  const submit=async e=>{e.preventDefault();setMessage('');if(!email.toLowerCase().endsWith('@gsdecorating.com')) return setMessage('Use your @gsdecorating.com work email.');const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:location.href}});setMessage(error?error.message:'Check your email for your secure sign-in link.')};
