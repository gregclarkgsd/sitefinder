...
function App({session,cloudEnabled}){
  ...
  const filtered=useMemo(()=>projects.filter(p=>{
    ...
    return (!query.trim()||search.includes(query.trim().toLowerCase()))
      ...
      && (!filters.recency||(filters.recency==='new'&&meta?.discovered_after_baseline&&age<=NEW_DAYS*86400000)||(filters.recency==='updated'&&changedAge<=NEW_DAYS*86400000&&age>86400000))
